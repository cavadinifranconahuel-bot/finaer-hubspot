...
const headerHtml = `<div style="background:#ffffff;padding:14px 32px;text-align:center;border-bottom:3px solid #f0f0f0;">
  <img src="${LOGO_URL}" alt="Finaer - Garantías para alquilar" style="height:52px;width:auto;display:inline-block;">
</div>`;

// Hero = banner image + event strip + registration heading (all in one widget,
// because the next DND slot is the FORM widget — so the heading must go HERE)
const heroHtml = `<div style="line-height:0;">
...
const whyHtml = `<div style="background:#ffffff;padding:36px 24px 40px;max-width:800px;margin:0 auto;">
...
const footerHtml = `<div style="background:#414042;color:#ffffff;padding:24px 32px;text-align:center;">
...
async function run() {
  console.log('Obteniendo landing SINOR...');
  const page = await apiRequest('GET', '/cms/v3/pages/landing-pages/392770971849');
  const ls = JSON.parse(JSON.stringify(page.layoutSections));

  // Widget assignments (corrected):
  // widget_1745254113596 → header logo
  // widget_1745255776476 → hero banner + event strip + form heading (all together, directly above form)
  // widget_1745253208703 → FORM (row 2 in DND — unchanged)
  // widget_1745255805864 → empty (form heading moved into hero above)
  // widget_1745254160831 → why attend
  // widget_1745255070582 → footer

  const richContent = {
    'widget_1745254113596': headerHtml,
    'widget_1745255776476': heroHtml,       // hero + heading merged
    'widget_1745255805864': '',             // cleared (heading moved to hero)
  ...
  console.log('Actualizando landing...');
  const result = await apiRequest('PATCH', '/cms/v3/pages/landing-pages/392770971849', {
    layoutSections: ls
  });

  if (result.id) {
    console.log('✅ Landing actualizada');
    console.log('   ID:     ' + result.id);
    console.log('   Estado: ' + result.currentState);
    console.log('   Editor: https://app-eu1.hubspot.com/pages/145725856/editor/' + result.id + '/content');
  } else {
    console.log('ERROR:', JSON.stringify(result).slice(0, 400));
  }
}
