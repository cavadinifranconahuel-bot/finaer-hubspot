...
exports.main = async (event, callback) => {
  ...
  try {
    ...
    // El primer resultado (ID mas bajo) es siempre el padre
    const padre = search.results[0];
    const hijos = search.results.slice(1);
    console.log('Padre:', padre.id, '| Hijos:', hijos.map(h => h.id).join(', '));

    for (const hijo of hijos) {
      // Paso 1: Eliminar asociaciones incorrectas (si existen)
      try {
        await client.apiRequest({
          method: 'DELETE',
          path: '/crm/v4/objects/tickets/' + padre.id + '/associations/tickets/' + hijo.id,
          body: [{ associationCategory: 'USER_DEFINED', associationTypeId: 46 }]
        });
      } catch(e) { /* no existia, ok */ }

      try {
        await client.apiRequest({
          method: 'DELETE',
          path: '/crm/v4/objects/tickets/' + hijo.id + '/associations/tickets/' + padre.id,
          body: [{ associationCategory: 'USER_DEFINED', associationTypeId: 45 }]
        });
      } catch(e) { /* no existia, ok */ }

      // Paso 2: Crear asociaciones correctas (idempotente - si ya existen no duplica)
      await client.apiRequest({
        method: 'POST',
      ...
      console.log('Padre ' + padre.id + ' <-> Hijo ' + hijo.id + ' OK');
    }
...
fetch('https://api.hubapi.com/automation/v4/flows/4315625700', {
  ...
  if (d.id) console.log('WF actualizado v3. ID:', d.id);
